...
| host | mongod WiredTiger cache | `MINHASH_MATCHING_FUNCTION_BATCH_SIZE` | `STORAGE_MATCHING_CACHE_MAX_ENTRIES` | `STORAGE_CACHE_FETCH_THREADS` | expected worker peak |
|---|---|---|---|---|---|
| 8 GiB / 2 cores | 3 GiB | 200 | 1,000,000 (~300 MB) | 2 | ~1.5–2 GiB |
| 16 GiB / 4 cores | 6 GiB | 500 | 2,000,000 (~630 MB) | 4 | ~2–3 GiB |
| 32 GiB / 8 cores | 12–15 GiB | 1000 | 5,000,000 (~1.5 GB) | 8 | ~3–5 GiB |
| 64 GiB / 16 cores | 24 GiB | 1000–5000 | 5,000,000 | 8 | ~5–8 GiB, room for 2–3 jobs |
...
These settings are strictly better and involve no trade-off:
...
| `MINHASH_POOL_MATCHING` | `False` | redundant once scoring is vectorised, and the source of run-to-run non-determinism |
...
* **Cache ceiling** costs ~314 bytes per retained function. Size it above your largest sample's
  candidate set; when it binds it silently halves the benefit, with nothing in the logs to say
  so.
...
* `STORAGE_CANDIDATE_ACCUMULATION`, `MINHASH_MATCHING_VECTORIZED`, `STORAGE_CACHE_FETCH_THREADS`
  and `STORAGE_MATCHING_CACHE_PERSIST` are **not yet part of an upstream MCRIT release**. On a
  stock release only the batch size, `BAND_MATCHES_REQUIRED` and the mongod cache size apply.
