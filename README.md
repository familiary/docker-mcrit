 ...
 * 2026-09-08: MCRIT 1.9.0 (correctness and operator-recovery release plus a large `getUniqueBlocks` speedup; **no migration, no re-index and no database shape change**, and matching results are unchanged at default configuration - verified byte-for-byte on an 11.6M-function corpus. **Rebuild the mcrit images**, and note that `config/` gains a **new file, `BandPresets.py`, which is required**: 1.9.0's `StorageConfig` imports `BAND_PRESETS` from it, and because this deployment bind-mounts `./config/` over the package's own config directory, a config folder without that file makes server and worker fail to import at startup - so **merge the regenerated `config/` rather than keeping your own copy wholesale**. The only intentional deviations from 1.9.0 stock remain `STORAGE_SERVER`/`QUEUE_SERVER` pointing at the `mongodb` service and `BAND_MATCHES_REQUIRED = 1`; the comment on the latter is updated, because previous bumps deferred that choice "until the reworked banding lands" and it has now landed - `k` is query-time, so one index serves every operating point, and the measured ones are hunt k=1, identification k=2-3, fast k=5. Which to ship as a default is a product decision, so the value is unchanged here. **One upgrade surprise worth knowing before you act on it**: 1.9.0 decides minhash staleness from a per-sample `minhash_smda_version` that did not exist before, and a sample without one counts as stale, so the first `/status` reports *every* sample as having stale minhashes whatever the true state - do not answer that by running `repair_minhashes`, which would rehash the whole corpus for hours; check whether they really are stale and, if they are current, record it with `setMinHashVersionForSamples(<running smda version>)` instead. Five one-time operator actions are available and none is automatic: `rebuild_picblockhash_index` (enables the fast `getUniqueBlocks` - 92 s to 0.11 s for one sample on the reference corpus; until it runs the previous full scan is used, so upgrading changes performance and never results), `recompute_family_stats`, `repair_minhashes`, `purgeEmptyBandDocuments()` and `db.functions.dropIndex("_picblockhashes.offset_1")`, which reclaims an index no query could use. New endpoints arrive with it, so MCRITweb needs a matching release to *use* them; MCRITweb 1.4.8 works unchanged against this version), MCRITweb 1.4.8
